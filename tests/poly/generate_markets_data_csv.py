...
def write_markets_to_csv(markets_list, csv_file="./data/markets_data.csv"):
    ...
    for market in markets_list:
        ...
        if "tokens" in market:
            csv_columns.update(
                {f"token_{key}" for token in market["tokens"] for key in token.keys()}
            )
        ...
        if market_data and "tokens" in market_data:
            for token in market_data["tokens"]:
                if token["outcome"].lower() == outcome.lower():
                    return token["price"]
        print(
            f"Price not found for condition_id: {condition_id} with outcome: {outcome}"
        )
    except Exception as e:
...
def create_condition_id_question_mapping():
    ...
    condition_id_question_map = {
        market["condition_id"]: market["question"] for market in markets_list
    }
            ...
            if market_data:
                for token in market_data["tokens"]:
                    if token["outcome"].lower() == side_a_outcome.lower():
                        ...
                        side_a_cost += price
                        side_a_info.append(
                            (market_data["question"], side_a_outcome, price)
                        )
            ...
            if market_data:
                for token in market_data["tokens"]:
                    if token["outcome"].lower() == side_b_outcome.lower():
                        ...
                        side_b_cost += price
                        side_b_info.append(
                            (market_data["question"], side_b_outcome, price)
                        )
